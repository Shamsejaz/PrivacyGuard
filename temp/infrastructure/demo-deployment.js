...
exports.demonstrateLambdaDeployment = demonstrateLambdaDeployment;
const lambda_registry_1 = require("../lambda-functions/lambda-registry");
/**
...
async function demonstrateLambdaDeployment() {
    ...
    const registry = (0, lambda_registry_1.getLambdaRegistry)();
    const functions = registry.getAllFunctions();
    console.log(`Found ${functions.length} Lambda functions for deployment:`);
    functions.forEach((func, index) => {
        console.log(`${index + 1}. ${func.metadata.functionName}`);
        console.log(`   Description: ${func.metadata.description}`);
        console.log(`   Actions: ${func.metadata.supportedActions.join(', ')}`);
        console.log(`   Risk Level: ${func.metadata.riskLevel}`);
        console.log(`   Rollback Supported: ${func.metadata.rollbackSupported ? '✅' : '❌'}`);
        console.log('');
    });
...
exports.default = demonstrateLambdaDeployment;
